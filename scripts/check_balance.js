...
for (let i = 0; i < s.length; i++) {
  ...
  if (inSingle || inDouble || inBack) {
    if (ch === '\\') { i++; col++; } // skip escaped char inside string
    continue;
  }
  ...
  if (ch === ')' || ch === '}' || ch === ']') {
    ...
    if (!last) { console.log(`UNMATCHED_CLOSING ${ch} at ${line}:${col}`); process.exit(2); }
    const match = (last.ch === '(' && ch === ')') || (last.ch === '{' && ch === '}') || (last.ch === '[' && ch === ']');
    if (!match) { console.log(`MISMATCH ${last.ch} opened at ${last.line}:${last.col} but closed by ${ch} at ${line}:${col}`); process.exit(3); }
  }
}
if (stack.length) {
  const last = stack[stack.length - 1];
  console.log(`UNCLOSED ${last.ch} opened at ${last.line}:${last.col}`);
  process.exit(4);
}
console.log('OK');
